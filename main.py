...
lowlightlogs = list(np.load(r'C:\Users\just\Documents\My Received Files\lowlightlogs.npy'))
# train = np.load(r'C:\Users\just\Documents\My Received Files\5th_nbtr9_train.npy')
picklefile = open(r'C:\Users\just\Documents\My Received Files\lighting_and_full_data.pickle', "rb")
...
picklefile = open(r'C:\Users\just\Downloads\train_full_data.pickle', "rb")
data_temp = pk.load(picklefile)
picklefile.close()
train_lognames = data_temp['Log_name']

train_vol = [a[1] for a in zip(data['Log_name'],data['Volume']) if a[0] in train_lognames]
train_spd = [a[1] for a in zip(data['Log_name'],data['Speed']) if a[0] in train_lognames]
train_gt = [a[1] for a in zip(data['Log_name'],data['Weight']) if a[0] in train_lognames]

picklefile = open(r'C:\Users\just\Downloads\validation_full_data.pickle', "rb")
data_temp = pk.load(picklefile)
picklefile.close()

val_lognames = data_temp['Log_name']
val_vol = [a[1] for a in zip(data['Log_name'],data['Volume']) if a[0] in val_lognames]
val_spd = [a[1] for a in zip(data['Log_name'],data['Speed']) if a[0] in val_lognames]
val_gt = [a[1] for a in zip(data['Log_name'],data['Weight']) if a[0] in val_lognames]

picklefile = open(r'C:\Users\just\Downloads\test_full_data.pickle', "rb")
data_temp = pk.load(picklefile)
picklefile.close()

test_lognames = data_temp['Log_name']
...
trainvol = np.transpose(np.concatenate([np.array([np.squeeze(np.array(a[1])), np.repeat(ind, len(a[1]))]) for ind, a in enumerate(zip(train_lognames, train_vol)) if a[0] not in lowlightlogs], axis=1))
trainspd = np.transpose(np.concatenate([np.array([np.squeeze(np.array(a[1]))]) for ind, a in enumerate(zip(train_lognames, train_spd)) if a[0] not in lowlightlogs], axis=1))
dfTrain_gt = pd.DataFrame(np.array([a[1] for a in zip(train_lognames, train_gt) if a[0] not in lowlightlogs]))
...
valvol = np.transpose(np.concatenate([np.array([np.squeeze(np.array(a[1])), np.repeat(ind, len(a[1]))]) for ind, a in enumerate(zip(val_lognames, val_vol)) if a[0] not in lowlightlogs], axis=1))
valspd = np.transpose(np.concatenate([np.array([np.squeeze(np.array(a[1]))]) for ind, a in enumerate(zip(val_lognames, val_spd)) if a[0] not in lowlightlogs], axis=1))
dfVal_gt = pd.DataFrame(np.array([a[1] for a in zip(val_lognames, val_gt) if a[0] not in lowlightlogs]))

testvol = np.transpose(np.concatenate([np.array([np.squeeze(np.array(a[1])), np.repeat(ind, len(a[1]))]) for ind, a in enumerate(zip(test_lognames, test_vol)) if a[0] not in lowlightlogs], axis=1))
...
if segment:
    ...
    loss_val = functools.partial(loss, features=x_val_in, label=y_val_in, model=model, elevspd = valspd.astype('float64'), weights=weights_val, grouper = None, regL2 = np.float64(-1.0), regL1 = np.float64(-1.0), x=np.float64(0), var_reg = np.float64(-tf.nn.relu(-var_reg_in)),std_reg = np.float64(-tf.nn.relu(-std_reg_in)), L1=L1)
...
## bound x
if monotonic:
    bounds = [(0, None) if not en in [3,4,5,len(x_init)-1] else (None,None) for en, i in enumerate(x_init)]
    # bounds = [(0,None)]
    x, min_val, info = fmin_l_bfgs_b(func=evaluator.loss, x0=np.float64(x_init), bounds=bounds,
